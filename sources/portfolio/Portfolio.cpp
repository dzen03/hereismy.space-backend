...
void Portfolio::map_urls(simple_http_server::Server& server) {
  ...
  server.MapUrl(
      "/api/photos", [&](const simple_http_server::Request& request) -> auto {
        ...
        if (map_category_ids.contains(name)) {
          ...
          for (++category_it; category_it != categories.end(); ++category_it) {
            body << ',';
            output_photo(*category_it, map_id_photo.at(*category_it), body);
          }
        }
  ...
  server.MapUrl(
      "/api/photo", [&](const simple_http_server::Request& request) -> auto {
        std::ostringstream body;
